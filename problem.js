    ...
    run() {
        while(true) {
            ...
            if(this.done.has(this.pointer)) {
                if(this.options.infiniteLoopWarning) console.log('INFINITE LOOP Problem solution',this.accumulator);
                break;
            }
...
for (let i = 0; i < code.length; i++) {
    ...
    if(element.instruction === 'nop' || element.instruction === 'jmp') {
        ...
        if(fixedProgram.isFinished) {
            console.log('bonus question solution ',fixedProgram.accumulator);
        }
    }
}
